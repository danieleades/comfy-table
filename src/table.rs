...
/// Each table consists of [Rows](Row), which in turn contain
...
impl Table {
    ...
    /// This is an alternative `fmt` function, which simply removes any trailing
    /// whitespaces. Trailing whitespaces often occur, when using tables
    /// without a right border.
    ...
    /// This is an alternative to `fmt`, but rather returns an iterator to each
    /// line, rather than one String separated by newlines.
    pub fn lines(&self) -> impl Iterator<Item = String> {
        build_table(self)
    }

    /// Set the header row of the table. This is usually the title of each
    /// column.
    ///
    ...
    /// Enforce a max width that should be used in combination with [dynamic
    /// content arrangement](ContentArrangement::Dynamic).
    ///
    /// This is usually not necessary, if you plan to output your table to a
    /// tty, since the terminal width can be automatically determined.
    ...
    ///
    /// This will be `Some(width)`, if the terminal width can be detected or if
    /// the table width is set via [set_table_width](Table::set_table_width).
    ///
    /// If neither is not possible, `None` will be returned.
    ///
    /// This implies that both the [Dynamic](ContentArrangement::Dynamic) mode
    /// and the [Percentage](crate::style::ColumnConstraint::Percentage)
    ...
    /// In case you are sure you don't want export tables to a tty or you
    /// experience problems with tty specific code, you can enforce a
    /// non_tty mode.
    ///
    /// This disables:
    ///
    /// - table_width lookup from the current tty
    /// - Styling and attributes on cells (unless you use
    ///   [Table::enforce_styling])
    ///
    /// If you use the [dynamic content
    /// arrangement](ContentArrangement::Dynamic), you need to set the width
    /// of your desired table manually with
    ...
    /// This behavior can be changed via [Table::force_no_tty] and
    /// [Table::use_stderr].
    #[cfg(feature = "tty")]
    ...
    ///
    /// Only useful if you forcefully disabled tty, but still want those fancy
    /// terminal styles.
    ///
    /// ```
    ...
    /// Returns whether the content of this table should be styled with the
    /// current settings and environment.
    ...
    ///
    /// **Attention:**
    /// This function should be called after at least one row (or the headers)
    /// has been added to the table. Before that, the columns won't
    /// initialized.
    ///
    /// If more constraints are passed than there are columns, any superfluous
    /// constraints will be ignored. ```
    /// use comfy_table::{CellAlignment, ColumnConstraint::*,
    ...
    ///     .set_constraints(vec![UpperBoundary(Fixed(15)),
    ...
    ///
    /// You can also write your own preset strings and use them with this
    /// function. There's the convenience method
    /// [Table::current_style_as_preset], which prints you a preset
    /// string from your current style configuration.
    ///
    /// The function expects the to-be-drawn characters to be in the same order
    /// as in the [TableComponent] enum.
    ///
    /// If the string isn't long enough, the default [ASCII_FULL] style will be
    /// used for all remaining components.
    ///
    ...
    ///
    /// A pure convenience method, so you're not force to fiddle with those
    /// preset strings yourself.
    ///
    /// ```
    ...
    /// Modify a preset with a modifier string from
    /// [modifiers](crate::style::modifiers).
    ///
    /// For instance, the
    /// [UTF8_ROUND_CORNERS](crate::style::modifiers::UTF8_ROUND_CORNERS)
    /// modifies all corners to be round UTF8 box corners.
    ///
    /// ```
    ...
    ///
    /// In case of a e.g. *BorderIntersection a whitespace will be used as
    /// placeholder, unless related borders and and corners are set to
    /// `None` as well.
    ///
    /// For example, if `TopBorderIntersections` is `None` the first row would
    /// look like this:
    ///
    /// ```text
    /// +------ ------+
    ...
    /// ```
    ///
    /// If in addition `TopLeftCorner`,`TopBorder` and `TopRightCorner` would be
    /// `None` as well, the first line wouldn't be displayed at all.
    ///
    /// ```
    /// use comfy_table::presets::UTF8_FULL;
    /// use comfy_table::Table;
    ...
    ///
    /// Though, if for instance all components of the left border are removed,
    /// the left border won't be displayed.
    ...
    /// Return a vector representing the maximum amount of characters in any
    /// line of this column.
    ///
    /// This is mostly needed for internal testing and formatting, but can be
    /// interesting if you want to see the widths of the longest lines for
    /// each column.
    ...
    /// Autogenerate new columns, if a row is added with more cells than
    /// existing columns.
    ...
    fn adjust_max_column_widths(&mut self, row: &Row) {
        ...
        for (index, width) in max_widths.iter().enumerate() {
            ...
            // We expect this column to exist, since we autoenerate columns just before
...
/// A dedicated struct is necessary, as data is usually handled by rows and
/// thereby stored in `Table::rows`. This type is returned by
/// [Table::column_cells_iter].
