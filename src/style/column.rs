...
///
/// They allow some control over Column widths as well as the dynamic
/// arrangement process.
///
...
pub enum ColumnConstraint {
    ...
    /// Use with caution! This can easily break your table, if the column's
    /// content is overly long.
    ContentWidth,
    /// Enforce a absolute width for a column.
    Absolute(Width),
    /// Specify a lower boundary, either fixed or as percentage of the total
    /// width.
    LowerBoundary(Width),
    /// Specify a upper boundary, either fixed or as percentage of the total
    /// width.
    UpperBoundary(Width),
...
pub enum Width {
    ...
    /// This can be used to specify an upper/lower boundary as well as a fixed
    /// size for the column.
    Fixed(u16),
    ...
    /// - you aren't using one of ContentArrangement::{Dynamic,
    ///   DynamicFullWidth}
    /// - the width of the table/terminal cannot be determined.
    Percentage(u16),
}
