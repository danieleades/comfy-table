...
/// These "elements" then get added one-by-one to the lines, until a line is
/// full. As soon as the line is full, we add it to the result set and start a
/// new line.
///
/// This is repeated until there're no more "elements".
///
/// Mid-element splits only occurs if a element doesn't fit in a single line by
/// itself.
pub fn split_line(line: &str, info: &ColumnDisplayInfo, delimiter: char) -> Vec<String> {
    ...
    // Also clone it and convert it into a Vec<String>. Otherwise we get some
    // burrowing problems due to early drops of borrowed values that need to be
    // inserted into `Vec<&str>`
    ...
    while let Some(next) = elements.pop() {
        ...
        // The remaining width for this column. If we are on a non-empty line, subtract
        // 1 for the delimiter.
        ...
        if added_length <= content_width {
            ...
            // Already complete the current line, if there isn't space for more than two
            // chars
        ...
        // Check, if there's enough space in the current line in case we decide to split
        // the element and only append a part of it to the current line.
        // If there isn't enough space, we simply push the current line, put the element
        // back on stack and start with a fresh line.
        ...
        // Ok. There's still enough space to fit something in (more than MIN_FREE_CHARS
        // characters) There are two scenarios:
        //
        // 1. The word is too long for a single line.
        //    In this case, we have to split the element anyways. Let's fill the
        // remaining space on    the current line with, start a new line and
        // push the remaining part on the stack. 2. The word is short enough to
        // fit as a whole into a line    In that case we simply push the current
        // line and start a new one with the current element
        ...
        if next_length > content_width {
            ...
            // It's supposed to be a new line, with a width of 1. However, the next char in
            // line is a multi-character UTF-8 symbol.
            //
            // Since a, for instance, two-character wide symbol doesn't fit into a
            // 1-character column, this code would loop endlessly. (There's no
            // legitimate way to split that word.)
            // Hence, we have to live with the fact, that this line will look broken, as we
            // put a two-character wide symbol into it.
...
/// It's used to check, whether another element can be added to the current
/// line. Otherwise the line will simply be left as it is and we start with a
/// new one. Two chars seems like a reasonable approach, since this would
/// require next element to be a single char + delimiter.
const MIN_FREE_CHARS: usize = 2;

/// Check if the current line is too long and whether we should start a new one
/// If it's too long, we add the current line to the list of lines and return a
/// new [String]. Otherwise, we simply return the current line and basically
/// don't do anything.
...
/// This needs some special logic, as we have to take multi-character UTF-8
/// symbols into account. When simply splitting at a certain char position, we
/// might end up with a string that's has a wider display width than allowed.
fn split_long_word(allowed_width: usize, word: &str) -> (String, String) {
    ...
    // That is, pushing the next character would result in the string being too
    // long.
    while let Some(c) = char_iter.peek() {
        ...
        // The user has to live with this, if they decide to add control characters or
        // some fancy stuff into their tables. This is considered undefined
        // behavior and we try to handle this to the best of our capabilities.
